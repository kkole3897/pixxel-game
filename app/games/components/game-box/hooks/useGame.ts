...
export function useGame(game: Game) {
  const releaseDateText = game.releaseDate != null ? `출시일 : ${dayjs(game.releaseDate).format('YYYY.MM.DD')}` : '출시 미정';
  ...
  const discountRate = (priceInfo.regular - priceInfo.discount) / priceInfo.regular;
