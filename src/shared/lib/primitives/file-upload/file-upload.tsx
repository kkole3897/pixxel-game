...
function getFilesFromEvent(
  context: FileUploadContextValue,
...
function syncInputElementAcceptedFiles(
  context: FileUploadContextValue,
  acceptedFiles: File[]
) {
  if (!context.inputElementRef.current) {
    return;
  }

  const dataTransfer = new DataTransfer();
  acceptedFiles.forEach((file) => dataTransfer.items.add(file));
  context.inputElementRef.current.files = dataTransfer.files;
}
...
function setFiles(
  ...
  syncInputElementAcceptedFiles(context, acceptedFiles);
}

function setFilesFromEvent(
  context: FileUploadContextValue,
  event: React.ChangeEvent<HTMLInputElement>
) {
...
type FileUploadTriggerProps = React.PropsWithChildren<
...
const FileUploadTrigger = forwardRef<HTMLButtonElement, FileUploadTriggerProps>(
  ({ children, onClick, ...props }, forwardedRef) => {
    ...
    const handleOpenFilePicker: React.MouseEventHandler<HTMLButtonElement> = (
      event
    ) => {
    ...
    return (
      <button {...props} ref={forwardedRef} type="button" onClick={handleClick}>
        {children}
      </button>
    );
  }
...
type FileUploadInputProps = React.PropsWithChildren<
  Omit<React.ComponentProps<'input'>, 'type' | 'multiple'>
>;

const FileUploadInput = forwardRef<HTMLInputElement, FileUploadInputProps>(
  (
    { style, name: nameProp, accept: acceptProp, onChange, ...props },
    forwardedRef
  ) => {
...
const FileUploadContext = ({ children }: FileUploadContextProps) =>
  children(useFileUploadContext());

type FileUploadItemGroupProps = React.PropsWithChildren<
...
type FileUploadItemProps = React.PropsWithChildren<
...
const FileUploadItem = forwardRef<HTMLLIElement, FileUploadItemProps>(
  ({ children, asChild, file, ...props }, forwardedRef) => {
    const Component = asChild ? Slot : 'li';

    const fileUploadContext = useFileUploadContext();

    const removeFile = () => {
      const newFiles = fileUploadContext.acceptedFiles.filter(
        (acceptedFile) => acceptedFile !== file
      );

      fileUploadContext.onFileChange?.(newFiles);
      setFiles(fileUploadContext, newFiles, fileUploadContext.rejectedFiles);
    };
    ...
    return (
      <FileUploadItemProvider value={context}>
        <Component {...props} ref={forwardedRef}>
          {children}
        </Component>
      </FileUploadItemProvider>
...
type FileUploadDeleteItemTriggerProps = Omit<
  React.PropsWithChildren<React.ComponentProps<'button'>>,
  'type'
>;

const FileUploadDeleteItemTrigger = forwardRef<
  HTMLButtonElement,
  FileUploadDeleteItemTriggerProps
>(
  (
    { onClick, children, 'aria-label': ariaLabelProp, ...props },
    forwardedRef
  ) => {
...
FileUploadDeleteItemTrigger.displayName =
  'PrimitiveFileUploadDeleteItemTrigger';
